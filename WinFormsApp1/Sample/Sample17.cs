    ...
    class Sample17
    {
        ...
        public static void Run(ResnetModel modelType)
        {
            ...
            if (ofd.ShowDialog() == DialogResult.OK)
            {
                ...
                //GPUを初期化
                foreach (KelpNet.CPU.FunctionStack<Real> resNetFunctionBlock in nn.FunctionBlocks)
                {
                    SwitchGPU(resNetFunctionBlock);
                }

                Debug.WriteLine("Model Loading done.");
                ...
                do
                {
                    ...
                    Debug.WriteLine("Result Time : " + (sw.ElapsedTicks / (Stopwatch.Frequency / (1000L * 1000L))).ToString("n0") + "μｓ");

                    int maxIndex = Array.IndexOf(result.Data, result.Data.Max());
                    Debug.WriteLine("[" + result.Data[maxIndex] + "] : " + classList[maxIndex]);
                } while (ofd.ShowDialog() == DialogResult.OK);
            }
        }

        static void SwitchGPU(KelpNet.CPU.FunctionStack<Real> functionStack)
        {
            for (int i = 0; i < functionStack.Functions.Length; i++)
            {
                if (functionStack.Functions[i] is KelpNet.CPU.Convolution2D<Real> || functionStack.Functions[i] is KelpNet.CPU.Linear<Real> || functionStack.Functions[i] is KelpNet.CPU.MaxPooling2D<Real>)
                {
