...
namespace WinFormsApp1
{
    ...
    class SingleBenchmark
    {
        ...
        {
            Debug.WriteLine("Bench data initializing...");
            ...
            Debug.WriteLine("◆" + linear.Name);

            sw.Restart();
            NdArray<Real>[] gradArrayCpu = linear.Forward(inputArrayCpu);
            sw.Stop();
            Debug.WriteLine("Forward [Cpu] : " + (sw.ElapsedTicks / (Stopwatch.Frequency / (1000L * 1000L))).ToString("n0") + "μｓ");
            ...
            sw.Stop();
            Debug.WriteLine("Backward[Cpu] : " + (sw.ElapsedTicks / (Stopwatch.Frequency / (1000L * 1000L))).ToString("n0") + "μｓ");
            ...
            if (linear.SetParallel(true))
            {
                ...
                sw.Stop();
                Debug.WriteLine("Forward [Gpu] : " + (sw.ElapsedTicks / (Stopwatch.Frequency / (1000L * 1000L))).ToString("n0") + "μｓ");
                ...
                sw.Stop();
                Debug.WriteLine("Backward[Gpu] : " + (sw.ElapsedTicks / (Stopwatch.Frequency / (1000L * 1000L))).ToString("n0") + "μｓ");
            }
            ...
            if (maxPooling2D.SetParallel(true))
            {
                ...
                //メモリ転送のみのため実装がない
                Debug.WriteLine("Backward[Gpu] : None");
            }
